...
export class NotificationsAlertService {
  ...
  getNotification() {
    this.subscriptions.push(
      this.httpClient.get(`example-data/mock-notification.json`).subscribe({
        next: (data: any) => {
  ...
  private closeNotification(data: any) {
    this.subscriptions.push(
      this.httpClient.patch(`${this.configService.getProperty('apiUrl')}`, data).subscribe({
        next: (_data: any) => {},
        error: (err: any) => {
          console.error(err);
        },
  ...
  deleteNotification(
    ...
    return notifications.filter(
      (notification: NotificationAlert) => notification.id !== notificationId
    );
  }
