...
export class DatatablesTwoService {
  ...
  getData<Type>(url: string, params: any, httpParams: HttpParams): Observable<Type> {
    ...
    if (this.pageParams) {
      ...
      httpParams = httpParams.set('pageParams.pageSize', this.pageParams.pageSize);
    }

    if (this.sortParams) {
      const sortByValue: string = this.sortParams.sortBy!.toString().trim();
      const defaultSortByValue = params.columns[0].data.trim();
      httpParams = httpParams.set(
        'sortParams.sortBy',
        sortByValue === '' ? defaultSortByValue : sortByValue
      );
      httpParams = httpParams.set('sortParams.ascending', this.sortParams.ascending!);
    }
